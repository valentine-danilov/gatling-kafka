  ...
  override protected def aroundSend(requestName: String, session: Session, message: ProducerRecord[Any, Any]): Validation[Around] = {
    for {
      ...
      new Around(
        before = () => {
          if (matchId != null) {
            tracker.track(matchId, clock.nowMillis, replyTimeoutMs, attributes.checks, session, next, requestName)
          }
        },
        after = () => {
          if (matchId == null) {
            ...
            if (updatedMatchId != null) {
              tracker.track(updatedMatchId, clock.nowMillis, replyTimeoutMs, attributes.checks, session, next, requestName)
            } else {
